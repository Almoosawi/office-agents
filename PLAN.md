...
> - r7 (2026-04-25): **CLIProxyAPI sidecar vendor + manager (Sprint 1 module 2.5)**. Pinned `v6.9.38` in `vendor/cliproxy/VERSION.json`; binary itself is `.gitignored`, fetched via `pnpm fetch:cliproxy` (sha256-verified) on first install. **Update flow with zero code changes**: `pnpm cliproxy:update` resolves the latest GitHub release, rewrites the pin (incl. `binary` field if upstream renames the executable — observed already: `CLIProxyAPI.exe` → `cli-proxy-api.exe`), recomputes the sha. Manager (`packages/bridge/src/sidecar/cliproxy.ts`) generates a fresh random API key per launch, writes a minimal `config.yaml` (only `host/port/auth-dir/api-keys`), spawns the binary with `-config <path>`, polls `/v1/models` until 200 (treating 401 as "server up"). Real-binary smoke test verified locally — cold start ~280ms. Sidecar adapter (`providers/sidecar.ts`) gained an `apiKey: () => string | undefined` supplier so the bridge launcher binds the manager's running key without re-creating the adapter. Default registry port updated to `8317` (CLIProxyAPI's default; was `7860`).
